<?php
/*
Template Name: Header - Заголовок
*/
...
<!--[if gt IE 8]><!--> <html class="no-js" lang="<?=((substr($_SERVER['REQUEST_URI'], 0, 4) === '/en/') ? 'en-us' : 'ru')?>"> <!--<![endif]-->
	...
	<?php endforeach; ?>
 		...
 		<meta property="og:title"         content="<?php if(get_field($title, 'category_'.$cat)): the_field($title, 'category_'.$cat ); else: wp_title(''); echo $prefix_title; endif; ?>" />
		
		<?if(is_category()): ?>
			<?php if(get_field($description)):?>
                <meta property="og:description" content="<?php the_field($description, 'category_'.$cat)?>">
            <?php else:?>
                <meta property="og:description" content="<? single_cat_title(); echo $prefix_description;?>">
            <?php endif;?>
		...
		<?else:?>
			...
			<?php else:?>
				<meta property="og:description" content="<?php wp_title(''); echo $prefix_description;?>">
			<?php endif;?>
		<?endif;?>
		...
		<?if(is_category()): ?>
			...
			<?php if(get_field($description)):?>
                <meta name="twitter:description " content="<?php the_field($description, 'category_'.$cat)?>">
            <?php else:?>
                <meta name="twitter:description " content="<? single_cat_title(); echo $prefix_description;?>">
            <?php endif;?>
			
		<?else:?>

			<?php if(get_field($description)):?>
				<meta name="twitter:description" content="<?php the_field($description)?>">
			<?php else:?>
				<meta name="twitter:description" content="<?php wp_title(''); echo $prefix_description;?>">
			<?php endif;?>
		...
		<!-- Yandex meta -->
		<meta name="yandex-verification" content="aedee3c950d6e13c" />
		...
		<script>
 			var OneSignal = window.OneSignal || [];
  			OneSignal.push(function() {
   				OneSignal.init({
     				appId: "b8c31b30-bde6-431e-91be-85cb186c2824",
   		 		});
 			 });
		</script>
	...
	";"+Math.random();</script>
		...
		<div id="person_data">
			<div class="center_wrap">
				<div class="dt person_data_center">
					<?php if ($cl==ru): ?>
					<span>Данный сайт <a href="http://undergroundexpert.info/soglasie-na-obrabotku-personalnyh-dannyh/">использует файлы «cookies»</a> с целью персонализации сервисов и повышения удобства пользования веб-сайтом.<br>«Cookies» представляют собой небольшие файлы, содержащие информацию о предыдущих посещениях веб-сайта.<br>Если вы не хотите использовать файлы «cookies», измените настройки браузера.</span>
					<?php ;else: ?>
					<span>This website uses <a href="http://undergroundexpert.info/en/consent-to-the-processing-of-personal-data/">files called cookies</a> to customise your experience and make the website more convenient to use. <br>Cookies are small files that contain information about previous visits to websites.<br>If you want to stop accepting cookies, please change your browser settings.</span>
					<?php ;endif; ?>
					<a class="close_person_data"></a>
				</div>
			</div>	
		</div> <!--/#person_data-->
		<?php ;endif; ?>
